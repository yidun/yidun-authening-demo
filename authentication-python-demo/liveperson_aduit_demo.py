...
class TextCheckAPIDemo(object):
    API_URL = "https://verify.dun.163.com/v1/liveperson/audit"
    VERSION = "v1"
...
if __name__ == "__main__":
    ...
    if code == 200:
        ...
        print("taskId=%s,认证结果:%s,原因详情:%s,图片类型:%s,抓取头像照片:%s,公安是否比对通过:%s,相似度得分:%s,具体请参考接口文档说明" %
              (result.get("taskId"), result.get("status"), result.get("reasonType"), result.get("picType"), result.get("avatar"),
               result.get("faceMatched"), result.get("similarityScore")))
    else:
