...
class TextCheckAPIDemo(object):
    API_URL = "https://verify.dun.163.com/v1/rp/check"
    VERSION = "v1"
...
if __name__ == "__main__":
    ...
    if code == 200:
        ...
        print("taskId=%s,实人认证结果:%s,人脸比对结果:%s,相似度得分:%s,详情信息:%s,具体请参考接口文档说明" %
              (result.get("taskId"), result.get("status"), result.get("faceMatched"), result.get("similarityScore"), result.get("reasonType")))
    else:
