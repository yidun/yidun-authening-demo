...
const (
	apiURL     = "https://verify.dun.163.com/v1/rp/check"
	version    = "v1"
...
func main() {
	params := url.Values{
		"name":     []string{"张三"},
		"cardNo":   []string{"341622123456141234"},
		"picType":  []string{"1"},
		"avatar":   []string{"http://www.xxx.com/xxx.jpg"},
	...
	if code == 200 {
		...
		faceMatched, _ := ret.Get("result").Get("faceMatched").String()
		similarityScore, _ := ret.Get("result").Get("similarityScore").String()
		reasonType, _ := ret.Get("result").Get("reasonType").String()
		fmt.Printf("taskId =%s,实人认证结果:%s,人脸比对结果:%s,相似度得分:%s,详情信息:%s,, 具体请参考接口文档说明\n", taskId, status, faceMatched, similarityScore, reasonType)
	} else {
