...
const (
	apiURL     = "https://verify.dun.163.com/v1/ocr/check"
	version    = "v1"
...
func main() {
	params := url.Values{
		"picType":      []string{"1"},
		"frontPicture": []string{"https://123.jpg"},
		"backPicture":  []string{"https://456.jpg"},
	}
	...
	if code == 200 {
		...
		fmt.Printf("taskId=%s,身份证OCR识别结果:%s,识别数据:%s, 具体请参考接口文档说明\n", taskId, status, ocrResponseDetail)
	} else {
