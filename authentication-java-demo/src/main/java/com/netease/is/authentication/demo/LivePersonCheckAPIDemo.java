 ...
 * @author yidun
 */
public class LivePersonCheckAPIDemo {
    /**
     * 产品密钥ID，产品标识
     */
     ...
     * 接口地址
     */
    private final static String API_URL = "https://verify.dun.163yun.com/v1/liveperson/recheck";
    /**
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (code == 200) {
            ...
            if (status == 1) {
                System.out.printf("taskId=%s，认证结果：通过%n", taskId);
            } else if (status == 2) {
                System.out.printf("taskId=%s，认证结果：不通过, 原因：%s%n", taskId, reasonType);
            } else {
                System.out.printf("taskId=%s，认证结果: %s,具体参见接口文档说明%n", taskId, status);
            }
        } else {
